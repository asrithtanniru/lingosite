...
export default function LivePage() {
  ...
  const previewLanguage: string = componentData.language || 'en'
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            ...
            <p className="text-sm text-foreground/50 mt-2">
              Created on {new Date(componentData.createdAt).toLocaleDateString()}
            </p>
          </CardContent>
        </Card>
        ...
        {isPreview ? (
          <div className="border-2 border-border rounded-base bg-white p-8 min-h-[600px]">
            <GeneratedSiteViewer
              sourceCode={componentData.code}
              localizedCode={componentData.localizedCode}
              language={previewLanguage}
              expectedLocalized={previewLanguage !== 'en'}
            />
          </div>
        ) : (
          <Card>
            ...
            <CardContent>
              <pre className="bg-secondary-background p-6 rounded-base overflow-auto text-sm border-2 border-border">
                <code>{componentData.localizedCode || componentData.code}</code>
              </pre>
            </CardContent>
          </Card>
        )}
